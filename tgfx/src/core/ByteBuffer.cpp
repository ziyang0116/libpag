...
bool ByteBuffer::cmpAnyOf(size_t size, const std::initializer_list<const void*>& bufList) {
  return std::any_of(bufList.begin(), bufList.end(), [this, size](const void* buf) {
    return memcmp(bytes + _position, buf, size) == 0;
  });
}
